...
:::danger 版本绑定的历史参考 —— 运行任何示例前请先读完

本页记录的是 **Sirius v1.0 世代**的 Python SDK，作为该世代用户的参考保留，不是当前固件的接口规范。

- `set_parameter` 可以**关闭电机力矩**、**解除力矩限幅**。力矩关闭后机器人会因自重瘫倒；解除限幅后其运动力量足以伤人或损坏自身。
- 运行任何示例前：清空机器人周围场地，把它放在稳定支撑面上或由人扶持，并确认其固件世代与本文档一致。
