...
| [Mid-level (II): Python API](./tutorial-basics/python_api.md) | This section introduces how to use Python SDK to control the movement status of your Sirius. You can try using Python to learn robot control and complete the secondary development of your Sirius by following the interfaces and instances we provide. **Sirius v1.0 generation — read the safety notice at the top of that page first.**|
|[Legacy Sirius v1.0 WebSocket API (retired)](./tutorial-basics/deploy_your_site.md) | This page is a retirement notice for the legacy v1.0 WebSocket API. It explains why that protocol was withdrawn and the safety boundaries to observe; it provides no current API instructions. |
