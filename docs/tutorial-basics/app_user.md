...
|-------|-------|
| After the connection is successful, the APP will go directly to the home page, and click **"Enter Remote Control"** to start and experience your wonderful interaction with Sirius. |![app_mode](./img/Quick_use_img/app_mode.jpg) |
...
| Steps | Reference pictures |
|------|-------|
| **Label 1**: Slide the joystick to make Sirius move and achieve gait movement.**Label 2**: Switch the speed mode between "Normal/Rapid" to control the gait speed of Sirius. | ![app_rocker](./img/Quick_use_img/app_rocker.jpg) |
...
| [Mid-level (II): Python API](./python_api.md) | This section introduces how to use Python SDK to control the movement status of your Sirius. You can try using Python to learn robot control and complete the secondary development of your Sirius by following the interfaces and instances we provide. **Sirius v1.0 generation — read the safety notice at the top of that page first.**|
|[Legacy Sirius v1.0 WebSocket API (retired)](./deploy_your_site.md) | This page is a retirement notice for the legacy v1.0 WebSocket API. It explains why that protocol was withdrawn and the safety boundaries to observe; it provides no current API instructions. |
