...
We will introduce Sparky's head User Interface (UI) here. Its head is not only the core of its vision and interaction, but also integrates a rich User Interface (UI) and surprises from R&D personnel of HENGBOT.
...
|----|----|---|
| **All operations after entering the interactive page** | Simply select "Exit" or "Enter" through the buttons on the left and right ears of Sparky, and confirm by tapping the area on the head. | ![head](./img/head.jpg) |

### 2.2 Color Recognition

> Color recognition usually involves image processing and machine learning techniques. Color recognition is like giving Sparky a "Color palette" that not only allows it to see the colors in the image, but also recognizes specific colors and marks the colors that the user is interested in.
...
| **Step 2: Prepare the recognition object**：After the function is activated, prepare a red ball or red paper box. Ensure that the object is completely within the field of view of the head camera, so that Sparky can automatically perform color recognition and tracking.| |
...
> Self-Balancing refers to the ability of a system or device to automatically adjust its posture or position to maintain a stable state; Imagine it's like Sparky having its own "Sense of Balance"; When it stands, whether it is lightly pushed or fine tuned in position, its internal system quickly senses these changes and restores balance by finely adjusting leg movements.
...
To observe how Sparky achieves self-balancing posture under different movements, please follow the steps below:
...
| **Step 2: Single-leg Lifting Test** Gently lift Sparky's single leg and observe how its other three legs slowly and steadily adjust its position to maintain the balance of the entire body.| |
| **Step 3: Dual-leg Lifting Test** Try to lift both the front left and right legs of Sparky at the same time, observe how its hind legs cleverly lower the height and lift the buttocks to maintain overall balance.||
|**Step 4: Customize Balance Actions** Now, you can freely try the actions you want to keep Sparky balanced. ||

These steps not only help you understand the self-balancing ability of Sparky, but also allow you to explore more creative gameplay. But remember to ensure that Sparky is in a safe environment during testing to avoid injury caused by losing balance.
...
| **Step 2: Experience Freedom Mode** After activating Freedom Mode, Sparky will stare at you like an observer, and its head will move along with your facial movements. At the same time, its legs will be in a semi-unloaded force of self-balancing state, so there is no need to worry about any impact on the robot dog even if there is a slight collision or compression. | |


### 2.5 Remote Control Mode  

Remote control mode is one of the core features of the UI on the Sparky's head, designed specifically for remote control of Sparky. In this mode, users can perform network settings to connect with Sparky's App, and use WebSocket API interface for deeper development.  
...
| **How to adjust the volume?** | We can control Sparky's barking by adjusting the volume.| ![ui_volume](./img/Quick_use_img/ui_volume.jpg) |
...
| **Check system information** | After activation, Sparky will automatically read system information | ![ui_system](./img/Quick_use_img/ui_system.jpg) |

So far, you have already learned and understood your Sparky on a basic level. It's the end of guidance. It's time for you to explore more interesting gameplay!